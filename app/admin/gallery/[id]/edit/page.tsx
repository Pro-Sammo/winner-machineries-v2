...
const categories = ["Manufacturing", "Products", "Facilities", "Quality", "Team", "Innovation", "Training"]
...
export default function EditGalleryImagePage() {
  ...
  // Fetch gallery image data on component mount
  useEffect(() => {
    const fetchGalleryImage = async () => {
      try {
        const response = await fetch(`/api/admin/gallery/${params.id}`)
        if (response.ok) {
          const galleryImage: GalleryImage = await response.json()
          setFormData({
            ...
            image: {
              ...
              publicId: galleryImage.image?.publicId || ""
            },
        ...
        alert("Failed to fetch gallery image data. Please try again.")
      } finally {
        setFetching(false)
      }
    }
